...
- A workspace start may mount at most **22 distinct host directories** — a conservative limit to avoid Apple `container` startup failures with large mount sets. Projects, `host.mounts`, agent profiles, [agent install mounts](#agent-install-mounts), tmux mounts, the herdr state mounts, and GitHub CLI mounts each count toward it.
