...
An explicit `host.mounts` entry (or an agent profile) at any of these container paths takes precedence, and pi-tin says so at start. A `workspace_state` path that overlaps one of these mounts is skipped with a warning: the path already persists via the mount, and snapshotting it would be destructive as well as redundant.
