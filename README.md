...
- **Structured errors.** In JSON mode, errors are emitted as a structured envelope on stderr — `{ "error": { "message", "code", … } }` — so an agent can read the machine-stable `code` (and any `remediation`, `validValues`, or `badInput` fields) instead of grepping the message text. An unknown command is a validation failure: exit 2 with code `unknown_command` and the valid command list in `validValues` (this also applies when --help follows an unknown command).
