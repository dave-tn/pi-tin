...
- **Interrupted open**: closing the terminal window (SIGHUP), a logout, or a `kill` mid-open ends the session and arms the same countdown before pi-tin exits, so an interrupted open never leaves a running workspace with nothing to reclaim it. The open stops there — it does not go on to attach to a terminal that is gone. No `workspace_state` snapshot is taken on that path; it needs container subprocesses pi-tin is in the middle of losing.
