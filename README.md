...
- **Interrupted open**: closing the terminal window (SIGHUP), a logout, or a `kill` while an open is in progress closes the session and arms the same countdown before pi-tin exits, so a workspace is never left running with nothing to reclaim it. The open is abandoned at that point — it does not go on to attach to a terminal that is gone.
