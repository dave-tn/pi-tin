...
Alongside the profile-declared entries, pi-tin persists a small set of binaries it owns through the same mechanism: the herdr server (see [herdr attach](#herdr-attach)) and the self-updating native agent installs (Claude Code, OpenCode — see [Workspaces](#workspaces)). These carry a larger copy deadline and a changed-check, so an unchanged binary costs a quick probe rather than a copy; the "inert data only" guidance above applies to the profile-declared entries, not to these.
...
- `tools` are installed at image build by one of two methods. **Claude Code and OpenCode** use their official native installers; each keeps itself current in-container with its own auto-updater, and the updated binary persists across container recreations via [workspace state](#workspace-state), so a fresh start resumes from the last updated version rather than reverting to the image bake. **Everything else** (Codex, Pi, Amp, Gemini CLI, and unrecognised packages) installs globally via `npm install -g`, then refreshes in the background on every open — including joins to a running workspace. The refresh installs into a separate prefix, so the current version stays available throughout; new sessions pick up the new version once it lands, and a failed refresh keeps the existing versions. The interactive `create` command writes only `name` and `package`. For known agents, pi-tin re-derives internal metadata (including the install method) from `package` at runtime. Manual tool entries should also use only `name` and `package`; extra keys are rejected.
...
- A workspace start may mount at most **22 distinct host directories** — a conservative limit to avoid Apple `container` startup failures with large mount sets. Projects, `host.mounts`, agent profiles, tmux mounts, the herdr state mount, and GitHub CLI mounts each count toward it.
...
- **Crash forensics**: every stop of a running container first snapshots the guest kernel log to `~/.config/pi-tin/diagnostics/<workspace>.dmesg.log` (best-effort, ~18 KB, latest container life only) — each container is its own VM, so without this a guest-side crash leaves no evidence once the container is deleted.
