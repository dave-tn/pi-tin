...
Native agents (Claude Code, OpenCode) are **not** installed at image build. On the first `open`, pi-tin checks in-container whether each is present and runs its official installer if not, showing elapsed time while it works; after that the agent's own auto-updater keeps it current and every update lands on the mount. A failed or interrupted install is not fatal — the open continues without that agent and retries on the next one. `Ctrl-C` during an install skips it and drops you into the shell. pi-tin never prunes old versions: whatever the agent's updater does about them on a host, it does here.
...
If an install ever ends up corrupt, delete the agent's directory under `~/.config/pi-tin/workspace-state/<workspace>/` and reopen — the installer runs fresh.

> [!WARNING]
> Mounting `~/.local/bin` shadows anything the image baked there at build time. The managed profiles are unaffected (their build-time tool installs run as root and land in `/root/.local/bin`), but a **custom** profile that installs user tools into `~/.local/bin` — `pip install --user`, `uv`, `pipx` — will not see them in a workspace with Claude Code or `attach: herdr`. Install such tools elsewhere, or mount them yourself with `host.mounts`.
...
- **`open --build`**: Forces an image rebuild on the next fresh start. If the workspace already has active sessions, pi-tin refuses and asks you to stop it first. A rebuild does not touch the native agents (Claude Code, OpenCode) — their installs live on [host mounts](#agent-install-mounts) and their own updaters own their freshness.
...
- **Last session exit**: When the last host-side `pi-tin open` session closes, pi-tin starts an auto-stop countdown using `stopAfterLastSession` (default `30s`). Reopening during that grace period cancels the pending stop unless a fresh restart is needed to apply config changes. On herdr workspaces the countdown is agent-aware: while any pane reports a working agent it re-arms instead of stopping, and the final stop snapshots workspace state first.
